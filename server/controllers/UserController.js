...
const userCredits = async (req, res) => {
    try {
        ...
        res.json({ success: true, credits: userData.creditBalance, user: { name: userData.firstName } });
...
const paymentRazorpay = async (req, res) => {
    try {
        // userId comes from the middleware; planId from the frontend
        ...
        // Create Transaction Data (Using userId instead of clerkId)
        const transactionData = {
            userId, 
            plan,
            amount,
        ...
        const options = {
            amount: amount * 100,
            currency: process.env.CURRENCY,
            receipt: newTransaction._id.toString(), // Ensure string ID
        };
...
export { userCredits, paymentRazorpay, verifyRazorpay, paymentStripe, verifyStripe };